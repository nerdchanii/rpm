...
executable lockfile, the linker creates only registry-resolved dependency links,
and no CLI flag targets a workspace. The planned manifest-discovery and resolver
...
to #223. The workspace lockfile v2 representation is described in
`lockfile/SPEC.md` by #146 / PR #217, while executable
parser/schema/runtime/replay/publication work remains deferred to #224.
Linker/integration work is tracked by #147/#149, and workspace lifecycle/recovery
...
| workspace package lockfile records | `lockfile/SPEC.md` | representation described with a known selection-branch provenance gap, implementation deferred: lockfile v2 specifies ordered root/member records, local/external identities, per-parent edges, selector provenance, production precedence, replay validation, and deterministic v1-to-v2 migration; the executable runtime remains on v1 and does not yet read, write, replay, or publish workspace records | #224 parser/schema, then runtime/replay/publication after #221 + #147 implementation + #149 + #155 trust policy |
| external dependency edges under a workspace root | `lockfile/SPEC.md`, `resolver/SPEC.md` | resolver contract defined; lockfile v2 representation is defined with a known provenance gap, implementation deferred: external nodes deduplicate by `<name>@<version>`, and the resolver preserves canonical request text, exact raw selector provenance, selection-branch/classification provenance, request kind, and origin or resolved parent; lockfile v2 serializes per-parent edges and `selector_kind`, but that field does not distinguish a matching `dist-tags.latest` branch from the resolver's `latest-root-version-fallback` branch when both select the same external target | #221; #224 schema/reconciliation |
...
| workspace member writes and recovery | `install/recovery/SPEC.md`, `install/scripts/SPEC.md`, `linker/SPEC.md` | implementation and workspace contract deferred: #147 owns link construction; #222 owns workspace transaction, mutation, rollback, and recovery behavior | #147; #222 |
...
- The discovery boundary (#145) defines supported declarations, portable glob
  ...
  executable fixtures. The specified #146 lockfile representation and #148
  targeting contract, plus the #147 linker track, consume the same member table
...
- Lockfile v2 representation and compatibility are owned by #146 / PR #217 in
  `lockfile/SPEC.md`; the representation is specified with the selection-branch
  provenance gap recorded in the table above. #224 owns executable
  implementation in two slices: parser/schema work may proceed from the #146
  representation, subject to that provenance reconciliation, while
  runtime/replay/publication requires #221's graph/preflight implementation,
  the #147 linker/extraction-validation implementation, the #149 end-to-end
  fixture, and the #155 trust-policy boundary. The current executable lockfile
  v1 still has no workspace-origin marker, so this audit does not imply runtime
  workspace lockfile support.
...
- The delivery order records the #145 discovery and #148 command-targeting
  contracts as complete. The #146 lockfile v2 representation is specified with
  the selection-branch provenance gap above. (1) #221 is the first
  implementation, delivering the validated discovery table and resolver roots
  with executable fixtures. (2) The follow-up tracks then respect their
  dependencies: the #147
  linker track (contract first, implementation after) requires the #145 and
  #146 contract inputs; #223 target resolution and dispatch requires #221
  plus the #148 and #151 CLI/diagnostics contracts and is independent of #147
  and #224; and #224 parser/schema work may proceed from the #146
  representation, subject to the provenance reconciliation above, while its
  runtime/replay/publication implementation requires #221 graph/preflight,
  #147 linker/extraction validation, #149's end-to-end fixture, and the #155
  trust-policy boundary. (3) #149 follows the relevant
  #145-#148 contracts and is a required input to the #224 runtime slice and
  lifecycle activation. (4) #222 is last and requires #145, #146, #147, #149,
  #155, #221, and the #224 runtime/replay/publication slice before activating
  workspace lifecycle/recovery.
  #223 consumes #148 without redefining #145 member identity or ordering or #151
  failure/diagnostic ownership. #222 keeps lifecycle, staging, and recovery
  ownership. No implementation track treats the #145, #146, or #148 contract
  inputs as a future delivery step.
