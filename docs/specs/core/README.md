...
| workspace package lockfile records | `lockfile/SPEC.md` | contract defined for planned v2, implementation deferred: ordered root/member records preserve origin, manifest path, name, and optional declared version; structured local identities remain distinct from external identities; v1 remains the current root-only format and rejects v2; replay and fresh v2 publication fail closed without #221's bound validated member table and #147's bound name/destination projection result | #224; #221/#147 prerequisite |
| external dependency edges under a workspace root | `lockfile/SPEC.md`, `registry/SPEC.md`, `install/cache/SPEC.md`, `resolver/SPEC.md` | contract defined for planned v2, implementation deferred: external nodes deduplicate by `<name>@<version>`, while every parent edge preserves source identity, canonical request text, exact raw selector provenance, selection-branch/classification provenance, request kind/relationship, origin or resolved parent, and resolved target; reviewed trusted lockfile facts carry selected registry provenance and required SHA-512 SRI, with canonical ordering, reachability, selector/target validation, exact archive-manifest identity, deterministic `.bin` collision precedence, descriptor-bound cache verification/publication, and deterministic no-refetch replay | #221; #224; #147 extraction/link prerequisite |
...
- Planned v2 external replay treats the exact reviewed lockfile bytes as trusted
  ...
  access, exact archive `package.json` name/version, and extraction from the same
  stable verified descriptor. Shasum-only records fail closed in v2 while v1
  keeps its current fallback. Cache and structural linker destinations must be
  injective under host case-folding, Unicode normalization, trailing-space/dot,
  and reserved-name semantics before mutation. Exact-name `.bin` producers are
  one intentional output slot with deterministic v2 last-writer precedence;
  distinct names that only collide under host projection remain invalid.
  #221/#147 own the local-member and linker projections, #147 owns package and
  ...
  validation evidence blocks both replay and fresh v2 publication before live
  mutation. #224 integrates these prerequisites in the v2 runtime without
