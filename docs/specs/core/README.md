...
- Planned v2 external replay treats an exact `TrustedLockfile` capability issued
  ...
  `package.json` name/version/ordinary-dependencies/bin/scripts provenance
  matched to the locked outgoing edges, and extraction from the same stable
  verified descriptor. Shasum-only and legacy
  ...
  proves the root-bound proof plus write-excluding lease or content-version CAS.
  Ordinary `renameat`/`renameat2` cannot qualify. This is an M7 implementation
  blocker for planned v2 lockfile publication and replay. #224 integrates these
  prerequisites in the v2 runtime without weakening or reimplementing them.
