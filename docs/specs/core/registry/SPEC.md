...
- The selected `tarball` is an absolute HTTPS URL with no user information,
  ...
  This encoder does not change semver validity. A selected version such as
  `1.0.0+build` remains governed by the resolver's semver rules and is encoded
  as `1.0.0%2Bbuild`; the literal `+` spelling is accepted only as the exact
  equivalent described below. Lowercase or double-encoded alternatives remain
  rejected.
