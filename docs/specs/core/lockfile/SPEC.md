...
resolution and publication through a temporary file plus same-directory atomic
replacement. There is no automatic downgrade from v2 to v1. A fresh root-only
resolution may continue to write v1 under the
...
The successful #221 discovery result supplies the validated workspace-root
identity, canonical member table, and root-relative lockfile destination. A v2
reader or fresh writer retains that validated root context for the lockfile
transaction and uses descriptor-relative, no-follow operations where the
selected environment provides them. It must not reconstruct an approved
destination from an unvalidated path or follow a symlink during preflight.

A candidate lockfile is created as a regular file in a transaction-owned
temporary location under an approved root. RPM writes, flushes, serializes, and
validates the complete candidate before it becomes visible as `rpm.lock`.
Candidate creation and cleanup are limited to files owned by this transaction;
failure preserves any existing `rpm.lock` and install output. Until #222's
lifecycle contract is active, the candidate is not provided to hooks. A visible
temporary pathname is not committed lockfile state.

Before publication, RPM revalidates lexical root and destination containment
and the destination entry. A symlink, non-regular entry, unexpected directory,
or out-of-root destination is rejected. Publication uses a same-directory
atomic rename or host-equivalent complete-file replacement relative to the
approved lockfile directory. An absent destination uses a no-replace path where
available; replacing an existing file is permitted only under the package
manager's normal update policy and after the destination passes the allowed
identity and type checks. The destination is never intentionally exposed as a
partial file.

The writer runs the graph, registry-provenance, archive, and install-staging
gates before lockfile publication, as described above. If an approved root,
destination, candidate, or serialized-byte read shows observed identity or
content drift before a mutation, publication fails, cleans only its temporary
file, and preserves prior outputs. These checks detect ordinary races. They do
not provide a cross-object compare-and-swap, global filesystem linearization,
or immunity from an uncoordinated same-user writer, `mmap` mutation, or mount
replacement after the last check. #224 owns the concrete candidate lifecycle,
publication implementation, and executable fixtures; #221 supplies the
validated workspace-discovery prerequisite.
...
If the selected execution environment cannot provide the required no-follow
containment or same-directory atomic publication operation, the operation must
return a stable unsupported-capability failure before mutation. This SPEC does
not claim that every current adapter provides or lacks those capabilities.
...
- planned #224 candidate creation and `rpm.lock` publication through the
  validated workspace-root context, with a complete temporary file, flush, and
  same-directory atomic publication; failures preserve the prior lockfile and
  remove only the transaction's temporary file;
- observed root, destination, candidate, and serialized-byte drift cases that
  abort publication, preserve the raced entries and prior outputs, and leave no
  partial final lockfile; the fixtures document the detection boundary without
  asserting cross-object CAS or immunity from uncoordinated same-user writes;
- absent, symlink, non-regular, traversal, and unexpected-destination cases
  rejected before mutation, plus recovery that retries from the prior committed
  state without deleting unrelated files;
