...
row's immutable #145 manifest snapshot. It uses the row's retained, validated
native member directory as the working directory and prepends its ordinary
target-local `node_modules/.bin` path to `PATH` according to the run-script
contract. Dispatch must not reconstruct a native path from `member_path_key` or
accept a path that leaves the canonical root. Immediately before launching each
selected member, the consumer validates the retained #145 parent/name mapping
and directory identity. A missing, renamed, replaced, or identity-mismatched
entry fails before launch. After that check, ordinary platform process and
shell semantics apply. The contract reports a later observed replacement or
binary lookup failure through the child process. Writes by trusted
workspace/store writers remain outside its concurrency guarantee. The contract
does not prescribe an isolated child-process filesystem view after launch.

The #223 launch adapter uses the validated member directory as the child
working directory and the ordinary target-local `.bin` path in `PATH`. It does
not need a shell-specific file-descriptor path or a separate process filesystem
view. If the retained mapping or directory changes before launch, the target
fails before its child starts. A later write is handled by normal process and
shell errors; this contract does not require a platform capability gate for an
unobservable change after launch.
...
The target list is fully resolved and validated before execution. The
...
- after M8 #151 and the adopting `run` decision define failure continuation,
  ...
  each target using its immutable manifest snapshot, root-confined working
  directory, and ordinary target-local `.bin` PATH setup; observed pre-launch
  identity drift must be reported before that target starts;
...
- a pre-launch identity fixture that replaces the member pathname or target
  directory before validation, proving the target fails before its child starts;
- a target-local `.bin` fixture that proves the selected member's ordinary
  `.bin` path is prepended and remains confined to the member, while any later
  shell lookup failure is reported through normal child-process behavior;
