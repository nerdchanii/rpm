...
ordering. This run SPEC owns the target-local script behavior: a member
invocation consumes script text from the selected #145 member-table snapshot,
uses that row's retained root-confined native directory, and prepends its
`node_modules/.bin`. It does not reopen a member manifest or derive filesystem
identity from `member_path_key` during dispatch.
...
parent/name mapping and directory identity. A missing, renamed, replaced, or
identity-mismatched entry fails before spawn. The adapter then launches with
the validated member directory as the working directory and its ordinary
target-local `.bin` path in `PATH`.
This is the normal package-manager boundary: the contract confines the path and
checks it before launch, while later writes by trusted workspace/store writers
are outside its concurrency guarantee and are reported through ordinary shell
or child-process errors. The adapter does not reconstruct a path from
`member_path_key`, and it does not prescribe an isolated child-process
filesystem view after launch. A late identity or binary lookup failure is an
execution-time safety failure for that target; it may occur after an earlier
target has run and does not imply that no child ran.
...
Member-targeted fixtures must cover the pre-launch identity check, root-confined
working-directory and target-local `.bin` setup, and an observed replacement
reported before spawn. They need not simulate special isolation after launch or
require a platform capability gate. Root-location fixtures must cover invocation from a
