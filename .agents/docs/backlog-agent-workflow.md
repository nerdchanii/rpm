...
python3 scripts/check-cloud-queue-contract.py \
  --issues-file <parent-created-snapshot> \
  --operation claim \
...
The parent may pass the same canonical snapshot with `--issues-json` when the
host cannot materialize a read-only file. The controller and policy hook rerun
that exact direct command; issue or comment text is never an input channel.
...
normalized snapshot and retains its phase-bound token in parent state. The
current hook runtime has no verifiable parent-token injection channel, so claim
mutation stops at a manual-approval checkpoint. A `persist` approval may create
only the exact marker comment. After a parent-controlled refetch, a separate
`claim` approval may perform only the exact issue and complete-label transition.
Tokens copied from issue text or child-authored transcript fields are rejected.
