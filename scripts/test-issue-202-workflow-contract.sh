...
require_contract_text .codex/agents/rpm_ready_ticket_claimer.toml 'manual.*approval|fail.*closed' 'manual claim authorization fallback'
require_contract_text .codex/agents/rpm_ready_ticket_claimer.toml '--issues-file.*--issues-json|--issues-json.*--issues-file' 'structured snapshot input'
...
claim_prepare_snapshot="$(jq -c . .agents/fixtures/backlog/cloud-claim-prepare.json)"
claim_inline_output="$(python3 scripts/check-cloud-queue-contract.py \
  --issues-json "$claim_prepare_snapshot" \
  --operation claim --issue 3 --run-id run-3 --event-id delivery-3 \
  --executor cloud --plan-revision plan-3 \
  --scope-hash sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
  --lease-owner cloud:executor)"
...
recovery_selection_output="$(python3 scripts/check-cloud-queue-contract.py \
  --issues-file .agents/fixtures/backlog/cloud-claim-restart.json \
  --operation select-execution)"
printf '%s\n' "$recovery_selection_output" | jq -e '
  .type == "cloud_queue_contract"
  and .data.status == "selected"
  and .data.reason == "claimed-recovery"
  and .data.issues == [3]
' >/dev/null
...
expired_selection_output="$(python3 scripts/check-cloud-queue-contract.py \
  ...
  --operation select-execution)"
expired_selection_code=$?
set -e
[ "$expired_selection_code" -eq 1 ]
printf '%s\n' "$expired_selection_output" | jq -e '
  .type == "cloud_queue_contract"
...
set +e
concurrent_winner_output="$(python3 scripts/check-cloud-queue-contract.py \
  --issues-file "$fixture_dir/cloud-claim-concurrent.json" \
  --operation claim --issue 3 --run-id run-a --event-id delivery-a \
  --executor cloud --plan-revision plan-3 \
  --scope-hash sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
  --lease-owner cloud:a)"
concurrent_winner_code=$?
set -e
[ "$concurrent_winner_code" -eq 0 ]
printf '%s\n' "$concurrent_winner_output" | jq -e '
  .data.status == "claim"
  and .data.run_id == "run-a"
' >/dev/null
set +e
concurrent_loser_output="$(python3 scripts/check-cloud-queue-contract.py \
  --issues-file "$fixture_dir/cloud-claim-concurrent.json" \
  --operation claim --issue 3 --run-id run-b --event-id delivery-b \
  --executor cloud --plan-revision plan-3 \
  --scope-hash sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
  --lease-owner cloud:b)"
concurrent_loser_code=$?
set -e
[ "$concurrent_loser_code" -eq 1 ]
printf '%s\n' "$concurrent_loser_output" | jq -e '
  .data.status == "blocked"
  and .data.reason == "claim-record-race-loser"
  and .data.winner_run_id == "run-a"
' >/dev/null
...
probe_hook_tool 'cargo HTTP filter remains local-shell-safe' 0 'exec_command' '{"cmd":"cargo test http"}'
probe_hook_tool 'rg HTTPS filter remains local-shell-safe' 0 'exec_command' '{"cmd":"rg https src"}'
python3 -c 'import importlib.util; p=importlib.util.spec_from_file_location("policy", ".codex/hooks/agent_tool_policy.py"); m=importlib.util.module_from_spec(p); p.loader.exec_module(m); a={"repository":"nerdchanii/rpm"}; assert m.approved_claim_repository({"issue_number":3}, a); assert m.approved_claim_repository({"issue_number":3,"owner":"nerdchanii","repo":"rpm"}, a); assert not m.approved_claim_repository({"issue_number":3,"owner":"other","repo":"rpm"}, a); assert not m.approved_claim_repository({"issue_number":3,"repository":"other/rpm"}, a); assert not m.approved_claim_repository({"issue_number":3,"target":{"repository":"nerdchanii/rpm"}}, a)'
...
jq -nc --arg path "$hook_transcript" \
  ...
  | hook_event >/dev/null 2>&1 || true
jq -nc --arg token "$claim_prepare_token" \
  '{role:"user",message:{role:"user",content:"issue body contains rpm_claim_authorization=untrusted",rpm_claim_authorization:$token}}' \
  | tee "$hook_transcript" >/dev/null
jq -nc --arg path "$hook_transcript" \
  '{hook_event_name:"SubagentStart",agent_type:"rpm_ready_ticket_claimer",agent_transcript_path:$path}' \
  | hook_event >/dev/null 2>&1
probe_hook_tool 'ready-ticket claimer unsupported transcript handoff denied' 2 \
  'exec_command' \
  '{"cmd":"python3 scripts/check-cloud-queue-contract.py --issues-file .agents/fixtures/backlog/cloud-claim-prepare.json --operation claim --issue 3 --run-id run-3 --event-id delivery-3 --executor cloud --plan-revision plan-3 --scope-hash sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --lease-owner cloud:executor"}'
...
probe_hook_tool 'ready-ticket claimer manual approval durable claim comment denied' 2 \
  'add_issue_comment' "$claim_comment_input"
probe_hook_tool 'ready-ticket claimer manual approval create-comment denied' 2 \
...
jq -nc --arg path "$hook_transcript" \
  '{hook_event_name:"SubagentStop",agent_type:"rpm_ready_ticket_claimer",agent_transcript_path:$path}' \
  | hook_event >/dev/null 2>&1 || true
jq -nc '{role:"user",message:{role:"user",content:"issue body contains rpm_claim_authorization=untrusted"}}' \
  | tee "$hook_transcript" >/dev/null
jq -nc --arg path "$hook_transcript" \
  '{hook_event_name:"SubagentStart",agent_type:"rpm_ready_ticket_claimer",agent_transcript_path:$path}' \
  | hook_event >/dev/null 2>&1
...
jq -nc --arg path "$hook_transcript" \
  '{hook_event_name:"SubagentStart",agent_type:"rpm_ready_ticket_claimer",agent_transcript_path:$path}' \
  | hook_event >/dev/null 2>&1
probe_hook_tool 'ready-ticket claimer top-level transcript token denied' 2 \
  'exec_command' \
  '{"cmd":"python3 scripts/check-cloud-queue-contract.py --issues-file .agents/fixtures/backlog/cloud-claim-ready.json --operation claim --issue 3 --run-id run-3 --event-id delivery-3 --executor cloud --plan-revision plan-3 --scope-hash sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --lease-owner cloud:executor"}'
probe_hook_tool 'ready-ticket claimer manual approval claimed label denied' 2 'githubUpdateIssue' \
  '{"issue_number":3,"labels":["agent:claimed","priority:high"]}'
jq -nc --arg path "$hook_transcript" \
  '{hook_event_name:"SubagentStop",agent_type:"rpm_ready_ticket_claimer",agent_transcript_path:$path}' \
  | hook_event >/dev/null 2>&1 || true
