...
def load_json(path: str | None = None, raw: str | None = None) -> dict[str, object]:
    if (path is None) == (raw is None):
        raise ValueError("provide exactly one JSON source")
    value = json.loads(Path(path).read_text() if path is not None else str(raw))
...
def claim_record_sort_key(record: dict[str, object]) -> tuple[datetime, str, str, str]:
    return (
        parse_timestamp(record["started_at"], "claim_record.started_at"),
        str(record["run_id"]),
        str(record["event_id"]),
...
def recovery_record(
    ...
    records = [run for run in runs if run.get("issue") == issue_number]
    if not records:
        return None, "missing-persisted-claim-record"
    active = []
    ...
    if active:
        return min(active, key=claim_record_sort_key), None
    if missing_marker:
        return None, "missing-persistence-marker"
...
def claim(
    ...
    winner = min(active_runs, key=claim_record_sort_key) if active_runs else None
    if matching_runs:
        persisted = matching_runs[0]
        if winner is not None and persisted is not winner:
            return {
                "status": "blocked",
                "reason": "claim-record-race-loser",
                "issue": issue_number,
    ...
    if winner is not None:
        return {
            ...
            "issue": issue_number,
            "conflicting_run_id": winner.get("run_id"),
            "conflicting_event_id": winner.get("event_id"),
            "conflicting_idempotency_key": winner.get("idempotency_key"),
        }
    if current in set(str(value) for value in contract.get("active_states", [])):
...
def select_execution(
    ...
    for issue, state in states:
        if state != "claimed":
            continue
        record, reason = recovery_record(fixture, issue, contract)
        if record is None:
            recovery_blockers.append({"number": issue.get("number"), "reason": reason})
        else:
            recovery.append(issue)
    if recovery_blockers:
        return {
            "status": "blocked",
            "reason": "claim-recovery-invalid",
            "invalid": recovery_blockers,
            "issues": [],
        }
    if recovery:
        recovery.sort(key=lambda issue: int(issue.get("number", 0)))
        return {
            "status": "selected",
            "reason": "claimed-recovery",
            "issues": [int(issue.get("number", 0)) for issue in recovery[:batch_limit]],
        }
    selected = [
        issue
        for issue, state in states
...
def main() -> int:
    ...
    fixture = load_json(args.issues_file, args.issues_json)
