...
def trusted_claim_authorization(
    ...
    """Fail closed until the host exposes a verifiable parent handoff field."""
    del event
    return None
...
def is_generic_dispatcher(tool: str, tool_input: object) -> bool:
    """Reject provider calls whose registered operation is not inspectable."""
    tokens = operation_tokens(tool)
    has_pull_request_read = (
        "pull" in tokens
        and "request" in tokens
        and any(token in MCP_READ_ACTIONS for token in tokens)
    )
    input_has_mutation = any(
        any(
            token in MCP_MUTATION_VERBS or token in MCP_HTTP_MUTATION_VERBS
            for token in operation_tokens(value)
        )
        for value in operation_values(tool_input)
    )
    if has_explicit_mcp_read_action(tool) and not input_has_mutation:
        return False
    if any(token in GENERIC_EXECUTOR_WORDS for token in tokens) and not has_pull_request_read:
        return True
    has_ambiguous_namespace = any(
        token in AMBIGUOUS_PROVIDER_NAMESPACES for token in tokens
    )
    has_mutation_action = any(
        token in MCP_MUTATION_VERBS
        or token in MCP_HTTP_MUTATION_VERBS
        or token in {"do", "perform"}
        for token in tokens
    ) or input_has_mutation
    return has_ambiguous_namespace and has_mutation_action
...
def uses_raw_network_client(text: str) -> bool:
    ...
    if re.search(
        ...
        return True
    if re.search(
        r"(?<![a-z0-9_])(?:http|https)(?![a-z0-9_])\s+"
...
def has_explicit_mcp_read_action(tool: str) -> bool:
    return any(token in MCP_READ_ACTIONS for token in operation_tokens(tool))
...
def mcp_mutation_kind(tool: str, tool_input: object) -> str | None:
    ...
    command_tokens = operation_tokens(tool)
    command = "_".join(command_tokens)
    ...
    primary_action = mcp_segment_action(command_tokens)
    if (
    ...
    if role == "rpm_idea_issue_creator":
        if kind == "project_add":
            ...
            configured_registration = (
                set(normalized)
                == {"issue_number", "owner", "project_number", "repository"}
                and normalized.get("project_number") == 7
                and isinstance(normalized.get("issue_number"), int)
                and normalized["issue_number"] > 0
                and normalized.get("repository") == "nerdchanii/rpm"
                and normalized.get("owner") == "@me"
            )
            return configured_registration or structured_project_target_allowed(tool_input)
        return (
            kind == "issue_create"
...
def approved_claim_repository(
    tool_input: object, authorization: dict[str, object] | None
) -> bool:
    """Bind optional issue-update targets to the controller repository."""
    if not isinstance(tool_input, dict) or not isinstance(authorization, dict):
        return False
    expected = authorization.get("repository")
    if not isinstance(expected, str) or "/" not in expected:
        return False
    expected_owner, expected_repo = expected.split("/", 1)
    normalized = {str(key).casefold(): value for key, value in tool_input.items()}
    target_keys = {"owner", "repo", "repository"}
    if target_keys & (collect_keys(tool_input) - set(normalized)):
        return False
    repository = normalized.get("repository")
    if repository is not None and repository != expected:
        return False
    owner = normalized.get("owner")
    repo = normalized.get("repo")
    if (owner is None) != (repo is None):
        return False
    if owner is not None and (owner != expected_owner or repo != expected_repo):
        return False
    return True
...
def check_tool(event: dict[str, object]) -> int:
    ...
    if is_github_tool(tool):
        ...
        if mutation is None:
            return 0
        if role == "rpm_ready_ticket_claimer" and authorization is None:
            return deny("rpm_ready_ticket_claimer lacks parent authorization")
        if role not in GITHUB_MUTATION_ROLES:
            return deny(
                f"{role} has read-only GitHub capability access; "
                f"detected mutation {mutation}"
            )
        return (
            0
    ...
    if is_shell_tool(tool):
        ...
        if uses_raw_network_client(command_text):
